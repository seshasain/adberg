...
const UseCases = () => {
  ...
  return (
    <section id="use-cases" className="py-20 bg-white" role="main">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {useCases.map((useCase, index) => (
            <div key={index} className="animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
              <Card className="h-full hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border-0 shadow-lg rounded-2xl">
                  ...
                  <ul className="space-y-2">
                    {useCase.benefits.map((benefit, benefitIndex) => (
                      <li key={benefitIndex} className="flex items-center justify-center">
                        ...
                        <span className="font-opensans text-sm text-gray-700">{benefit}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
              ...
              {['TechCorp', 'StyleBrand', 'FitLife', 'GreenEarth'].map((brand, index) => (
                <div key={index} className="text-center">
                  <div className="w-16 h-16 bg-white rounded-2xl shadow-md flex items-center justify-center mx-auto mb-2">
                    <span className="font-montserrat font-bold text-gray-400 text-xs">{brand}</span>
                  </div>
                  <p className="font-opensans text-xs text-gray-600">Trusted Partner</p>
                </div>
              ))}
            </div>
          </div>
