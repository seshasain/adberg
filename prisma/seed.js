...
async function main() {
  ...
  for (const tool of tools) {
    // The model name in Prisma Client is lowercase despite being uppercase in the schema
    await prisma.$executeRaw`INSERT INTO "tools" ("id", "name", "description", "category", "path", "icon", "is_active", "created_at", "updated_at") 
      VALUES (gen_random_uuid(), ${tool.name}, ${tool.description}, ${tool.category}, ${tool.path}, ${tool.icon}, true, now(), now())`
  }
